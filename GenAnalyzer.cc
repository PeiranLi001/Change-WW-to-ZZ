...
GenAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for(vector<reco::GenParticle>::const_iterator genps_it = genps_coll->begin(); genps_it != genps_coll->end(); genps_it++)
  {
    ...
    /*	Quarks from Z-boson	*/
    if ( abs(genps_it->pdgId())<7 && (genps_it->mother()->pdgId()==23) && genps_it->status()==23 && genps_it->isHardProcess()) {
      if (Verbose_) std::cout << "In Z loop... " << std::endl;
      Zquarks.SetPtEtaPhiE(genps_it->pt(), genps_it->eta(), genps_it->phi(), genps_it->energy());
      Vec_zJET.push_back(Zquarks);
      pdgID_.push_back(genps_it->pdgId());
    } /* END if condition of zjETs */
  ...
  if (Vec_Photons.size()==2 &&  Vec_zJET.size()==4)
  {
    ...
    Vec_Higgs.push_back(Vec_Zboson[0]+Vec_Zboson[1]);
    ...
    gen_leading_ZBoson_Phi_  = Vec_Zboson[0].Phi();
    gen_leading_ZBoson_M_  = Vec_Zboson[0].M();

    gen_Subleading_ZBoson_Pt_  = Vec_Zboson[1].Pt();
    gen_Subleading_ZBoson_Eta_  = Vec_Zboson[1].Eta();
    gen_Subleading_ZBoson_Phi_  = Vec_Zboson[1].Phi();
    gen_Subleading_ZBoson_M_  = Vec_Zboson[1].M();

    gen_HiggsZZ_Pt_  = Vec_Higgs[0].Pt();
    gen_HiggsZZ_Eta_  = Vec_Higgs[0].Eta();
    gen_HiggsZZ_Phi_  = Vec_Higgs[0].Phi();
    gen_HiggsZZ_M_  = Vec_Higgs[0].M();

    gen_HiggsGG_Pt_  = Vec_Higgs[1].Pt();
    gen_HiggsGG_Eta_  = Vec_Higgs[1].Eta();
